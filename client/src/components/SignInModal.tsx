...
const SignUpModal = () => {
  ...
  const handleSignIn =  async(e : React.FormEvent) => {
    ...
    try {
      const message = await SignUpUser({username , password , selectedCategory })
      console.log(message)
      navigate('/home')
    }
    catch (err) {
  ...
  return (
    <>
      {/* Background Overlay */}
      <div className="fixed inset-0 bg-black/15 z-40"  onClick={()=> console.log("cliclk")} />
      ...
      transition={{ duration: 0.1, ease: "linear",delay :0 }} 
        ...
        style={{height : containerHeight}}>
          ...
          <form onSubmit={isSigningIn ? handleSignIn : handleLogin}>
            ...
            {userNotExist && <p className="text-red-500 text-xs text-center">User Not Found. Try Again or Sign up</p>}
            <button
              type="submit"
