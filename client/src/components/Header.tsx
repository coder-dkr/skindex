...
const Header: React.FC = () => {
  ...
  return (
    <header className="w-full h-24 bg-[#785BF8] flex items-center justify-between px-11 sticky top-0">
      <div>
        <img src="/assets/skindex.svg" />
      </div>

      <div className="flex w-[41rem] items-center bg-white rounded-full  px-7 gap-2">
        <button className="cursor-pointer"><img src="/assets/search.svg" /></button>
            <input
                type="text"
                value={text}
      ...
      <button onClick={handleOnClick} className="text-white cursor-pointer font-[14px]">
        {token ? 'Logout' : 'Login'}
      </button>
